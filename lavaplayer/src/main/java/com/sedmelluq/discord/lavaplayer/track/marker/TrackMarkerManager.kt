package com.sedmelluq.discord.lavaplayer.track.marker
...
class TrackMarkerManager {
     ...
     */
    suspend fun set(marker: TrackMarker?, currentTimecode: Long) {
     ...
     */
    suspend fun trigger(state: MarkerState?) {
     ...
     */
    suspend fun checkPlaybackTimecode(timecode: Long) {
     ...
     */
    suspend fun checkSeekTimecode(timecode: Long) {
        val marker = current.value
        if (marker != null && timecode >= marker.timecode) {
    ...
    private suspend fun trigger(marker: TrackMarker, state: MarkerState) {
